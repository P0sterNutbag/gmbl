...
func _process(delta: float) -> void:
	...
	if Globals.player.gun:
		...
		if Globals.player.gun.max_ammo > 0:
			var current_ammo = float(Globals.player.gun.gun_stats.ammo)
			var max_ammo = float(Globals.player.gun.max_ammo)
			var ammo_percentage = current_ammo / max_ammo
			# Calculate which sprite to show (0-19 for 20 sprites)
			var sprite_index = int(ammo_percentage * 19)
			# Set the region rect to show the correct sprite (28 pixels wide each)
