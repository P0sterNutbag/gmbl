...
func _ready() -> void:
	...
	#gun_holder.process_mode = Node.PROCESS_MODE_DISABLED
	hide()
...
func _process(delta: float) -> void:
	...
	if death_camera.current:
		...
		# match gun
		var gun = gun_holder.get_child(0)
		if player.gun and gun.scene_file_path != PlayerStats.gun.physical_item.resource_path:
			gun.queue_free()
			gun = PlayerStats.gun.gun_object.instantiate()
			gun_holder.add_child(gun)
		# animation
		if player.gun_state == player.gun_states.ads:
			animation_player.play("IdleAim")
		elif player.velocity.length() > 0.5:
			if player.gun:
